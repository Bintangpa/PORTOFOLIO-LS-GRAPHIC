        ...
        <form action="{{ route('portfolio') }}" method="GET" class="search-form">
            <div class="search-input">
                <input type="text" name="search" class="form-control" 
                       placeholder="{{ \App\Models\WebsiteSetting::getValue('portfolio_search_placeholder', 'Cari portofolio berdasarkan judul, deskripsi, kategori, atau klien...') }}" 
                       value="{{ request('search') }}">
            </div>
            <button type="submit" class="btn search-btn">
                <i class="fas fa-search me-2"></i>{{ \App\Models\WebsiteSetting::getValue('portfolio_search_button', 'Cari') }}
            </button>
            @if(request('search') || request('category'))
                <a href="{{ route('portfolio') }}" class="btn btn-outline-secondary">
                    <i class="fas fa-times me-2"></i>{{ \App\Models\WebsiteSetting::getValue('portfolio_reset_button', 'Reset') }}
                </a>
            @endif
        </form>
        ...
        @if(request('search') || request('category'))
            <div class="mt-3">
                <small class="text-muted">
                    @if(request('search') && request('category'))
                        {{ \App\Models\WebsiteSetting::getValue('portfolio_search_results_for', 'Menampilkan hasil pencarian untuk:') }} <strong>"{{ request('search') }}"</strong> 
                        dalam kategori <strong>"{{ request('category') }}"</strong>
                    @elseif(request('search'))
                        {{ \App\Models\WebsiteSetting::getValue('portfolio_search_results_for', 'Menampilkan hasil pencarian untuk:') }} <strong>"{{ request('search') }}"</strong>
                    @elseif(request('category'))
                        {{ \App\Models\WebsiteSetting::getValue('portfolio_search_category', 'Menampilkan portofolio kategori:') }} <strong>"{{ request('category') }}"</strong>
                    @endif
                    ({{ $portfolios->total() }} {{ \App\Models\WebsiteSetting::getValue('portfolio_search_results_found', 'hasil ditemukan') }})
                </small>
            </div>
        @endif
        ...
        @if($categories->count() > 0)
            <div class="mt-3">
                <div class="d-flex flex-wrap align-items-center gap-2">
                    <small class="text-muted me-2">{{ \App\Models\WebsiteSetting::getValue('portfolio_filter_label', 'Filter kategori:') }}</small>
                    <a href="{{ route('portfolio') }}" class="category-filter-btn {{ !request('category') ? 'active' : '' }}">
                        {{ \App\Models\WebsiteSetting::getValue('portfolio_filter_all', 'Semua') }}
                    </a>
            ...
            @if($portfolios->hasPages())
                <div class="pagination-container">
                    ...
                    <div class="pagination-info">
                        {{ \App\Models\WebsiteSetting::getValue('portfolio_pagination_showing', 'Menampilkan') }} {{ $portfolios->firstItem() }} - {{ $portfolios->lastItem() }} {{ \App\Models\WebsiteSetting::getValue('portfolio_pagination_of', 'dari') }} {{ $portfolios->total() }} {{ \App\Models\WebsiteSetting::getValue('portfolio_pagination_portfolios', 'portofolio') }}
                    </div>
                </div>
            @endif
        @else
            <div class="empty-state">
                @if(request('search'))
                    ...
                    <h4>{{ \App\Models\WebsiteSetting::getValue('portfolio_no_results_title', 'Tidak ada hasil ditemukan') }}</h4>
                    <p>{{ \App\Models\WebsiteSetting::getValue('portfolio_no_results_message', 'Coba gunakan kata kunci yang berbeda atau') }} <a href="{{ route('portfolio') }}">{{ \App\Models\WebsiteSetting::getValue('portfolio_no_results_link', 'lihat semua portofolio') }}</a></p>
                @else
                    <i class="fas fa-folder-open fa-5x"></i>
                    <h4>{{ \App\Models\WebsiteSetting::getValue('portfolio_empty_title', 'Belum ada portofolio') }}</h4>
                    <p>{{ \App\Models\WebsiteSetting::getValue('portfolio_empty_message', 'Portofolio akan segera ditampilkan di sini') }}</p>
                @endif
            </div>
        @endif
    </div>
            ...
            <div class="container">
                <div class="social-media-container">
                    <div class="social-header text-center mb-4">
                        <h3>{{ \App\Models\WebsiteSetting::getValue('portfolio_social_title', 'Ikuti Kami') }}</h3>
                        <p class="lead">{{ \App\Models\WebsiteSetting::getValue('portfolio_social_subtitle', 'Dapatkan update terbaru dari karya-karya kami') }}</p>
                    </div>
                    
                    @if($hasAnySocial)
                        <div class="social-links">
                            @if(\App\Models\WebsiteSetting::getValue('portfolio_social_instagram'))
                                <a href="{{ \App\Models\WebsiteSetting::getValue('portfolio_social_instagram') }}" target="_blank" class="social-icon-link social-instagram" title="Instagram">
                                    <i class="fab fa-instagram"></i>
                                </a>
                            @endif
                            
                            @if(\App\Models\WebsiteSetting::getValue('portfolio_social_tiktok'))
                                <a href="{{ \App\Models\WebsiteSetting::getValue('portfolio_social_tiktok') }}" target="_blank" class="social-icon-link social-tiktok" title="TikTok">
                                    <i class="fab fa-tiktok"></i>
                                </a>
                            @endif
                            
                            @if(\App\Models\WebsiteSetting::getValue('portfolio_social_whatsapp'))
                                <a href="https://wa.me/{{ \App\Models\WebsiteSetting::getValue('portfolio_social_whatsapp') }}" target="_blank" class="social-icon-link social-whatsapp" title="WhatsApp">
                                    <i class="fab fa-whatsapp"></i>
                                </a>
                            @endif
                            
                            @if(\App\Models\WebsiteSetting::getValue('portfolio_social_email'))
                                <a href="mailto:{{ \App\Models\WebsiteSetting::getValue('portfolio_social_email') }}" class="social-icon-link social-email" title="Email">
                            ...
                            <div class="alert alert-info d-inline-block">
                                ...
                                @auth
                                    @if(auth()->user()->isAdmin())
                                        <a href="{{ route('admin.portfolio-page.index') }}" class="alert-link">Konfigurasi di sini</a>
                                    @endif
                                @endauth
                            </div>
                        </div>
